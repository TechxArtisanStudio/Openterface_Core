...
static void test_checksum_zero_data(void) {
    uint8_t data[14] = {0};
    ASSERT_EQ_INT(0x00, op_input_checksum(data, 14), "all zero checksum");
}

static void test_checksum_single_byte(void) {
    uint8_t data[1] = {0x42};
    ASSERT_EQ_INT(0x00, op_input_checksum(data, 1), "single byte (len-1=0, sum=0)");
}

static void test_checksum_overflow_truncation(void) {
    uint8_t data[3] = {0xFF, 0xFF, 0x00};
    ASSERT_EQ_INT(0xFE, op_input_checksum(data, 3), "overflow truncation");
}
...
static void test_checksum_known_kb_packet(void) {
    ...
    pkt[7] = 0x04;
    ASSERT_EQ_INT(0x10, op_input_checksum(pkt, 14), "known keyboard checksum");
}
...
static void test_kb_packet_header(void) {
    ...
    ASSERT_EQ_INT(OP_INPUT_CMD_KB, pkt[3], "command byte");
    ASSERT_EQ_INT(0x08, pkt[4], "data length");
}
...
static void test_kb_packet_single_key(void) {
    ...
    ASSERT_EQ_INT(0x00, pkt[12], "slot 6 empty");
}
...
static void test_kb_packet_combined_modifiers(void) {
    ...
    ASSERT_EQ_INT(0x00, pkt[9], "slot 3 empty");
}
...
static void test_kb_packet_six_keys(void) {
    ...
    ASSERT_EQ_INT(0x07, pkt[10], "slot 4");
    ASSERT_EQ_INT(0x08, pkt[11], "slot 5");
    ASSERT_EQ_INT(0x09, pkt[12], "slot 6");
}
...
static void test_press_release_basic(void) {
    ...
    ASSERT_EQ_INT(0x00, out[14 + 5], "release modifier = 0");
    ASSERT_EQ_INT(0x00, out[14 + 7], "release key = 0");
    ASSERT_EQ_INT(0x00, out[14 + 8], "release slot 2 = 0");
}
...
static void test_char_lowercase(void) {
    ...
    for (c = 'a'; c <= 'z'; c++) {
        ...
        ASSERT_EQ_INT(0, shift, "lowercase no shift");
    }
    ASSERT_EQ_INT(0x04, op_input_hid_code_from_char('a', &shift), "'a' = 0x04");
    ASSERT_EQ_INT(0x1D, op_input_hid_code_from_char('z', &shift), "'z' = 0x1D");
}
...
static void test_char_uppercase(void) {
    ...
    for (c = 'A'; c <= 'Z'; c++) {
        ...
        ASSERT_EQ_INT(1, shift, "uppercase needs shift");
    }
}
...
static void test_char_digits(void) {
    ...
    for (c = '0'; c <= '9'; c++) {
        ...
        ASSERT_EQ_INT(0, shift, "digit no shift");
    }
    ASSERT_EQ_INT(0x27, op_input_hid_code_from_char('0', &shift), "'0' = 0x27");
    ASSERT_EQ_INT(0x1E, op_input_hid_code_from_char('1', &shift), "'1' = 0x1E");
    ASSERT_EQ_INT(0x26, op_input_hid_code_from_char('9', &shift), "'9' = 0x26");
}
...
static void test_char_shifted_symbols(void) {
    ...
    ASSERT_EQ_INT(0x1F, op_input_hid_code_from_char('@', &shift), "'@' HID");
    ASSERT_EQ_INT(1, shift, "'@' needs shift");
    ASSERT_EQ_INT(0x20, op_input_hid_code_from_char('#', &shift), "'#' HID");
    ASSERT_EQ_INT(1, shift, "'#' needs shift");
    ASSERT_EQ_INT(0x25, op_input_hid_code_from_char('*', &shift), "'*' HID");
    ASSERT_EQ_INT(1, shift, "'*' needs shift");
}

static void test_char_unshifted_punctuation(void) {
    int shift;
    ASSERT_EQ_INT(0x36, op_input_hid_code_from_char(',', &shift), "',' HID");
    ASSERT_EQ_INT(0, shift, "',' no shift");
    ASSERT_EQ_INT(0x2D, op_input_hid_code_from_char('-', &shift), "'-' HID");
    ASSERT_EQ_INT(0, shift, "'-' no shift");
    ASSERT_EQ_INT(0x37, op_input_hid_code_from_char('.', &shift), "'.' HID");
    ASSERT_EQ_INT(0, shift, "'.' no shift");
    ASSERT_EQ_INT(0x38, op_input_hid_code_from_char('/', &shift), "'/' HID");
    ASSERT_EQ_INT(0, shift, "'/' no shift");
    ASSERT_EQ_INT(0x33, op_input_hid_code_from_char(';', &shift), "';' HID");
    ASSERT_EQ_INT(0, shift, "';' no shift");
    ASSERT_EQ_INT(0x2E, op_input_hid_code_from_char('=', &shift), "'=' HID");
    ASSERT_EQ_INT(0, shift, "'=' no shift");
}
...
static void test_char_shifted_punctuation(void) {
    ...
    ASSERT_EQ_INT(0x33, op_input_hid_code_from_char(':', &shift), "':' HID");
    ASSERT_EQ_INT(1, shift, "':' needs shift");
    ASSERT_EQ_INT(0x34, op_input_hid_code_from_char('"', &shift), "'\"' HID");
    ASSERT_EQ_INT(1, shift, "'\"' needs shift");
    ASSERT_EQ_INT(0x36, op_input_hid_code_from_char('<', &shift), "'<' HID");
    ASSERT_EQ_INT(1, shift, "'<' needs shift");
    ASSERT_EQ_INT(0x37, op_input_hid_code_from_char('>', &shift), "'>' HID");
...
static void test_char_brackets(void) {
    int shift;
    ASSERT_EQ_INT(0x2F, op_input_hid_code_from_char('[', &shift), "'[' HID");
    ASSERT_EQ_INT(0, shift, "'[' no shift");
    ASSERT_EQ_INT(0x30, op_input_hid_code_from_char(']', &shift), "']' HID");
    ASSERT_EQ_INT(0, shift, "']' no shift");
    ASSERT_EQ_INT(0x2F, op_input_hid_code_from_char('{', &shift), "'{' HID");
    ASSERT_EQ_INT(1, shift, "'{' needs shift");
    ASSERT_EQ_INT(0x30, op_input_hid_code_from_char('}', &shift), "'}' HID");
...
static void test_char_special_keys(void) {
    ...
    ASSERT_EQ_INT(0x35, op_input_hid_code_from_char('`', &shift), "'`' HID");
    ASSERT_EQ_INT(0, shift, "'`' no shift");
    ASSERT_EQ_INT(0x34, op_input_hid_code_from_char('\'', &shift), "'\\'' HID");
    ASSERT_EQ_INT(0, shift, "'\\'' no shift");
    ASSERT_EQ_INT(0x64, op_input_hid_code_from_char('\\', &shift), "'\\\\' HID");
    ASSERT_EQ_INT(0, shift, "'\\\\' no shift");
}
...
static void test_char_out_of_range(void) {
    ...
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\t', &shift), "tab invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\r', &shift), "CR invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\x00', &shift), "NULL char invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\x1F', &shift), "0x1F invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\x7F', &shift), "DEL invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\x80', &shift), "0x80 invalid");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_char('\xFF', &shift), "0xFF invalid");
}

static void test_char_null_shift_ptr(void) {
    int hid = op_input_hid_code_from_char('A', NULL);
    ASSERT_EQ_INT(0x04, hid, "'A' HID with NULL shift ptr");
}
...
static void test_name_all_letters(void) {
    ...
    ASSERT_EQ_INT(0x05, op_input_hid_code_from_name("B"), "B");
    ASSERT_EQ_INT(0x06, op_input_hid_code_from_name("C"), "C");
    ASSERT_EQ_INT(0x0D, op_input_hid_code_from_name("J"), "J");
...
static void test_name_all_digits(void) {
    ...
    ASSERT_EQ_INT(0x1F, op_input_hid_code_from_name("2"), "2");
    ASSERT_EQ_INT(0x26, op_input_hid_code_from_name("9"), "9");
}
...
static void test_name_action_keys(void) {
    ...
    ASSERT_EQ_INT(0x39, op_input_hid_code_from_name("Caps"), "Caps");
}

static void test_name_function_keys(void) {
    ASSERT_EQ_INT(0x3A, op_input_hid_code_from_name("F1"), "F1");
    ASSERT_EQ_INT(0x3B, op_input_hid_code_from_name("F2"), "F2");
    ASSERT_EQ_INT(0x3F, op_input_hid_code_from_name("F6"), "F6");
    ASSERT_EQ_INT(0x44, op_input_hid_code_from_name("F11"), "F11");
...
static void test_name_navigation_keys(void) {
    ASSERT_EQ_INT(0x49, op_input_hid_code_from_name("Insert"), "Insert");
    ASSERT_EQ_INT(0x4A, op_input_hid_code_from_name("Home"), "Home");
    ASSERT_EQ_INT(0x4B, op_input_hid_code_from_name("PageUp"), "PageUp");
    ASSERT_EQ_INT(0x4C, op_input_hid_code_from_name("Delete"), "Delete");
    ASSERT_EQ_INT(0x4D, op_input_hid_code_from_name("End"), "End");
    ASSERT_EQ_INT(0x4E, op_input_hid_code_from_name("PageDown"), "PageDown");
}

static void test_name_navigation_aliases(void) {
    ASSERT_EQ_INT(0x4B, op_input_hid_code_from_name("PgUp"), "PgUp alias");
    ASSERT_EQ_INT(0x4E, op_input_hid_code_from_name("PgDn"), "PgDn alias");
}
...
static void test_name_numpad(void) {
    ...
    ASSERT_EQ_INT(0x59, op_input_hid_code_from_name("Numpad1"), "Numpad1");
    ASSERT_EQ_INT(0x61, op_input_hid_code_from_name("Numpad9"), "Numpad9");
    ASSERT_EQ_INT(0x63, op_input_hid_code_from_name("NumpadDot"), "NumpadDot");
    ASSERT_EQ_INT(0x54, op_input_hid_code_from_name("NumpadSlash"), "NumpadSlash");
    ASSERT_EQ_INT(0x55, op_input_hid_code_from_name("NumpadAsterisk"), "NumpadAsterisk");
    ASSERT_EQ_INT(0x56, op_input_hid_code_from_name("NumpadMinus"), "NumpadMinus");
    ASSERT_EQ_INT(0x57, op_input_hid_code_from_name("NumpadPlus"), "NumpadPlus");
    ASSERT_EQ_INT(0x58, op_input_hid_code_from_name("NumpadEnter"), "NumpadEnter");
    ASSERT_EQ_INT(0x67, op_input_hid_code_from_name("NumpadEquals"), "NumpadEquals");
    ASSERT_EQ_INT(0x53, op_input_hid_code_from_name("NumLock"), "NumLock");
}

static void test_name_system_keys(void) {
    ASSERT_EQ_INT(0x65, op_input_hid_code_from_name("App"), "App");
    ASSERT_EQ_INT(0x47, op_input_hid_code_from_name("ScrollLock"), "ScrollLock");
    ASSERT_EQ_INT(0x46, op_input_hid_code_from_name("PrtSc"), "PrtSc");
    ASSERT_EQ_INT(0x48, op_input_hid_code_from_name("Pause"), "Pause");
}

static void test_name_invalid(void) {
    ASSERT_EQ_INT(-1, op_input_hid_code_from_name("NoSuchKey"), "unknown key");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_name(""), "empty string");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_name(NULL), "NULL");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_name("f1"), "lowercase f1");
    ASSERT_EQ_INT(-1, op_input_hid_code_from_name("enter"), "lowercase enter");
}
...
static void test_label_known_keys(void) {
    ...
    ASSERT_EQ_STR("F1", op_input_hid_code_label(0x3A), "label F1");
    ASSERT_EQ_STR("Ctrl", op_input_hid_code_label(0xE0), "label Ctrl");
}
...
static void test_token_special_tags(void) {
    ASSERT_EQ_INT(0x28, op_input_parse_token("<ENTER>").hid_code, "<ENTER>");
    ASSERT_EQ_INT(0x29, op_input_parse_token("<ESC>").hid_code, "<ESC>");
    ASSERT_EQ_INT(0x2A, op_input_parse_token("<BACK>").hid_code, "<BACK>");
    ASSERT_EQ_INT(0x2B, op_input_parse_token("<TAB>").hid_code, "<TAB>");
    ASSERT_EQ_INT(0x2C, op_input_parse_token("<SPACE>").hid_code, "<SPACE>");
}

static void test_token_arrow_tags(void) {
    ASSERT_EQ_INT(0x50, op_input_parse_token("<LEFT>").hid_code, "<LEFT>");
    ASSERT_EQ_INT(0x4F, op_input_parse_token("<RIGHT>").hid_code, "<RIGHT>");
    ASSERT_EQ_INT(0x52, op_input_parse_token("<UP>").hid_code, "<UP>");
    ASSERT_EQ_INT(0x51, op_input_parse_token("<DOWN>").hid_code, "<DOWN>");
}

static void test_token_edit_tags(void) {
    ASSERT_EQ_INT(0x4C, op_input_parse_token("<DEL>").hid_code, "<DEL>");
    ASSERT_EQ_INT(0x4C, op_input_parse_token("<DELETE>").hid_code, "<DELETE>");
    ASSERT_EQ_INT(0x49, op_input_parse_token("<INS>").hid_code, "<INS>");
    ASSERT_EQ_INT(0x49, op_input_parse_token("<INSERT>").hid_code, "<INSERT>");
}

static void test_token_nav_tags(void) {
    ASSERT_EQ_INT(0x4A, op_input_parse_token("<HOME>").hid_code, "<HOME>");
    ASSERT_EQ_INT(0x4D, op_input_parse_token("<END>").hid_code, "<END>");
    ASSERT_EQ_INT(0x4B, op_input_parse_token("<PGUP>").hid_code, "<PGUP>");
    ASSERT_EQ_INT(0x4B, op_input_parse_token("<PAGEUP>").hid_code, "<PAGEUP>");
    ASSERT_EQ_INT(0x4E, op_input_parse_token("<PGDN>").hid_code, "<PGDN>");
    ASSERT_EQ_INT(0x4E, op_input_parse_token("<PAGEDOWN>").hid_code, "<PAGEDOWN>");
}

static void test_token_function_keys(void) {
    ASSERT_EQ_INT(0x3A, op_input_parse_token("<F1>").hid_code, "<F1>");
    ASSERT_EQ_INT(0x3B, op_input_parse_token("<F2>").hid_code, "<F2>");
    ASSERT_EQ_INT(0x45, op_input_parse_token("<F12>").hid_code, "<F12>");
}

static void test_token_modifier_tags_ignored(void) {
    ASSERT_EQ_INT(-1, op_input_parse_token("<CTRL>").hid_code, "<CTRL> no key");
    ASSERT_EQ_INT(-1, op_input_parse_token("</CTRL>").hid_code, "</CTRL> no key");
    ASSERT_EQ_INT(-1, op_input_parse_token("<SHIFT>").hid_code, "<SHIFT> no key");
    ASSERT_EQ_INT(-1, op_input_parse_token("<ALT>").hid_code, "<ALT> no key");
    ASSERT_EQ_INT(-1, op_input_parse_token("<CMD>").hid_code, "<CMD> no key");
    ASSERT_EQ_INT(-1, op_input_parse_token("<WIN>").hid_code, "<WIN> no key");
}

static void test_token_delay_tags(void) {
    ASSERT_EQ_INT(-1, op_input_parse_token("<DELAY1S>").hid_code, "<DELAY1S> ignored");
    ASSERT_EQ_INT(-1, op_input_parse_token("<DELAY500MS>").hid_code, "<DELAY500MS> ignored");
    ASSERT_EQ_INT(-1, op_input_parse_token("<DELAY>").hid_code, "<DELAY> ignored");
}

static void test_token_invalid(void) {
    ASSERT_EQ_INT(-1, op_input_parse_token("<UNKNOWN>").hid_code, "unknown tag");
    ASSERT_EQ_INT(-1, op_input_parse_token("???").hid_code, "garbage");
    ASSERT_EQ_INT(-1, op_input_parse_token("").hid_code, "empty");
    ASSERT_EQ_INT(-1, op_input_parse_token(NULL).hid_code, "NULL");
}
...
static void test_macro_ctrl_c(void) {
    ...
    ASSERT_EQ_INT(1, n, "Ctrl+C count");
    ASSERT_EQ_INT(0x06, tokens[0].hid_code, "C key");
...
static void test_macro_cmd_v(void) {
    ...
    ASSERT_EQ_INT(1, n, "Cmd+V count");
    ASSERT_EQ_INT(0x19, tokens[0].hid_code, "V key");
...
static void test_macro_win_r(void) {
    ...
    ASSERT_EQ_INT(1, n, "Win+R count");
    ASSERT_EQ_INT(0x15, tokens[0].hid_code, "R key");
    ASSERT_EQ_INT(OP_INPUT_MOD_GUI, tokens[0].modifiers, "GUI modifier");
}
...
static void test_macro_ctrl_alt_del(void) {
    ...
    ASSERT_EQ_INT(1, n, "Ctrl+Alt+Del count");
    ASSERT_EQ_INT(0x4C, tokens[0].hid_code, "Del key");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL | OP_INPUT_MOD_ALT, tokens[0].modifiers, "Ctrl|Alt");
}
...
static void test_macro_cmd_shift_z(void) {
    ...
    ASSERT_EQ_INT(0x1D, tokens[0].hid_code, "Z key");
    ASSERT_EQ_INT(OP_INPUT_MOD_GUI | OP_INPUT_MOD_SHIFT, tokens[0].modifiers, "GUI|Shift");
}

static void test_macro_plain_text(void) {
    op_input_parsed_token_t tokens[32];
    int n = op_input_macro_parse("abc", tokens, 32);
    ASSERT_EQ_INT(3, n, "'abc' count");
    ASSERT_EQ_INT(0x04, tokens[0].hid_code, "a");
    ASSERT_EQ_INT(0x05, tokens[1].hid_code, "b");
    ASSERT_EQ_INT(0x06, tokens[2].hid_code, "c");
    ASSERT_EQ_INT(0, tokens[0].modifiers, "a no modifier");
}
...
static void test_macro_case_insensitive(void) {
    ...
    ASSERT_EQ_INT(tokens_lo[0].hid_code, tokens_up[0].hid_code, "A vs a same HID");
    ASSERT_EQ_INT(tokens_lo[1].hid_code, tokens_up[1].hid_code, "B vs b same HID");
    ASSERT_EQ_INT(tokens_lo[2].hid_code, tokens_up[2].hid_code, "C vs c same HID");
}

static void test_macro_shifted_symbol(void) {
    op_input_parsed_token_t tokens[32];
    int n = op_input_macro_parse("Hi!", tokens, 32);
    ASSERT_EQ_INT(3, n, "'Hi!' count");
    ASSERT_EQ_INT(0, tokens[0].modifiers & OP_INPUT_MOD_SHIFT, "'H' no shift");
...
static void test_macro_delay_skipped(void) {
    ...
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[0].modifiers, "Ctrl preserved");
}
...
static void test_macro_null_empty(void) {
    op_input_parsed_token_t tokens[32];
    ASSERT_EQ_INT(0, op_input_macro_parse(NULL, tokens, 32), "NULL input");
    ASSERT_EQ_INT(0, op_input_macro_parse("", tokens, 32), "empty input");
    ASSERT_EQ_INT(0, op_input_macro_parse("abc", NULL, 32), "NULL output");
    ASSERT_EQ_INT(0, op_input_macro_parse("abc", tokens, 0), "max=0");
    ASSERT_EQ_INT(0, op_input_macro_parse("abc", tokens, -1), "max<0");
}

static void test_macro_max_truncation(void) {
    op_input_parsed_token_t tokens[3];
    int n = op_input_macro_parse("abcdefghij", tokens, 3);
    ASSERT_EQ_INT(3, n, "max=3 truncates");
    ASSERT_EQ_INT(0x04, tokens[0].hid_code, "a");
    ASSERT_EQ_INT(0x05, tokens[1].hid_code, "b");
    ASSERT_EQ_INT(0x06, tokens[2].hid_code, "c");
}
...
static void test_macro_unclosed_modifier(void) {
    ...
    ASSERT_EQ_INT(2, n, "unclosed Ctrl count");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[0].modifiers, "A has Ctrl");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[1].modifiers, "B has Ctrl");
}

static void test_macro_close_without_open(void) {
    op_input_parsed_token_t tokens[32];
    int n = op_input_macro_parse("A</CTRL>B", tokens, 32);
    ASSERT_EQ_INT(2, n, "close-without-open count");
    ASSERT_EQ_INT(0, tokens[0].modifiers, "A no modifier");
    ASSERT_EQ_INT(0, tokens[1].modifiers, "B no modifier");
}

static void test_macro_double_modifier_idempotent(void) {
    op_input_parsed_token_t tokens[32];
    int n = op_input_macro_parse("<CTRL><CTRL>A</CTRL></CTRL>", tokens, 32);
    ASSERT_EQ_INT(1, n, "double Ctrl count");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[0].modifiers, "Ctrl still 0x01");
}

static void test_macro_complex_mixed(void) {
    op_input_parsed_token_t tokens[32];
    int n = op_input_macro_parse("<CTRL>cd /tmp<ENTER></CTRL>", tokens, 32);
    ASSERT_EQ_INT(8, n, "complex mixed count");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[0].modifiers, "c has Ctrl");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL, tokens[7].modifiers, "Enter has Ctrl");
    ASSERT_EQ_INT(0x28, tokens[7].hid_code, "Enter key");
}
...
static void test_tokenize_plain_text(void) {
    ...
    ASSERT_EQ_INT(3, n, "plain text count");
    ASSERT_EQ_INT(1, spans[0].length_utf8, "a length");
    ASSERT_EQ_INT(1, spans[1].length_utf8, "b length");
    ASSERT_EQ_INT(1, spans[2].length_utf8, "c length");
}

static void test_tokenize_single_tag(void) {
    op_input_script_token_span_t spans[16];
    int n = op_input_script_tokenize("<CTRL>", spans, 16);
    ASSERT_EQ_INT(1, n, "single tag count");
    ASSERT_EQ_INT(6, spans[0].length_utf8, "<CTRL> length");
}

static void test_tokenize_mixed(void) {
    op_input_script_token_span_t spans[16];
    int n = op_input_script_tokenize("A<ENTER>B", spans, 16);
    ASSERT_EQ_INT(3, n, "mixed count");
    ASSERT_EQ_INT(1, spans[0].length_utf8, "A");
    ASSERT_EQ_INT(7, spans[1].length_utf8, "<ENTER>");
    ASSERT_EQ_INT(1, spans[2].length_utf8, "B");
}

static void test_tokenize_closing_tag(void) {
    op_input_script_token_span_t spans[16];
    int n = op_input_script_tokenize("</CTRL>", spans, 16);
    ASSERT_EQ_INT(1, n, "closing tag count");
    ASSERT_EQ_INT(7, spans[0].length_utf8, "</CTRL> length");
}

static void test_tokenize_utf8_multibyte(void) {
    op_input_script_token_span_t spans[16];
    int n = op_input_script_tokenize("\xe4\xb8\xad", spans, 16);
    ASSERT_EQ_INT(1, n, "UTF-8 char count");
    ASSERT_EQ_INT(3, spans[0].length_utf8, "UTF-8 length");
}

static void test_tokenize_null(void) {
    op_input_script_token_span_t spans[16];
    ASSERT_EQ_INT(0, op_input_script_tokenize(NULL, spans, 16), "NULL input");
    ASSERT_EQ_INT(0, op_input_script_tokenize("abc", NULL, 16), "NULL output");
    ASSERT_EQ_INT(0, op_input_script_tokenize("abc", spans, 0), "max=0");
}
...
static void test_tokenize_max_truncation(void) {
    ...
    ASSERT_EQ_INT(1, n, "max=1");
}

static void test_tokenize_illegal_tag(void) {
    op_input_script_token_span_t spans[16];
    int n = op_input_script_tokenize("<abc", spans, 16);
    ASSERT_EQ_INT(4, n, "illegal tag as chars");
}
...
static void test_modifier_constants(void) {
    ASSERT_EQ_INT(0x00, OP_INPUT_MOD_NONE,   "NONE = 0x00");
    ASSERT_EQ_INT(0x01, OP_INPUT_MOD_LCTRL,  "LCTRL = 0x01");
    ASSERT_EQ_INT(0x02, OP_INPUT_MOD_LSHIFT, "LSHIFT = 0x02");
    ASSERT_EQ_INT(0x04, OP_INPUT_MOD_LALT,   "LALT = 0x04");
    ASSERT_EQ_INT(0x08, OP_INPUT_MOD_LGUI,   "LGUI = 0x08");
    ASSERT_EQ_INT(0x10, OP_INPUT_MOD_RCTRL,  "RCTRL = 0x10");
    ASSERT_EQ_INT(0x20, OP_INPUT_MOD_RSHIFT, "RSHIFT = 0x20");
    ASSERT_EQ_INT(0x40, OP_INPUT_MOD_RALT,   "RALT = 0x40");
    ASSERT_EQ_INT(0x80, OP_INPUT_MOD_RGUI,   "RGUI = 0x80");
}

static void test_modifier_aliases(void) {
    ASSERT_EQ_INT(OP_INPUT_MOD_LCTRL,  OP_INPUT_MOD_CTRL,  "CTRL alias");
    ASSERT_EQ_INT(OP_INPUT_MOD_LSHIFT, OP_INPUT_MOD_SHIFT, "SHIFT alias");
    ASSERT_EQ_INT(OP_INPUT_MOD_LALT,   OP_INPUT_MOD_ALT,   "ALT alias");
    ASSERT_EQ_INT(OP_INPUT_MOD_LGUI,   OP_INPUT_MOD_GUI,   "GUI alias");
}
...
static void test_modifier_non_overlapping(void) {
    int all = OP_INPUT_MOD_LCTRL | OP_INPUT_MOD_LSHIFT | OP_INPUT_MOD_LALT | OP_INPUT_MOD_LGUI
            | OP_INPUT_MOD_RCTRL | OP_INPUT_MOD_RSHIFT | OP_INPUT_MOD_RALT | OP_INPUT_MOD_RGUI;
    ASSERT_EQ_INT(0xFF, all, "all mods = 0xFF");
    ASSERT_EQ_INT(0, OP_INPUT_MOD_LCTRL & OP_INPUT_MOD_LSHIFT, "CTRL & SHIFT = 0");
    ASSERT_EQ_INT(0, OP_INPUT_MOD_LALT  & OP_INPUT_MOD_LGUI,   "ALT & GUI = 0");
}
...
static void test_mouse_rel_left_click(void) {
    ...
    ASSERT_EQ_INT(11, len, "mouse rel length");
    ASSERT_EQ_INT(0x01, pkt[5], "relative mode");
    ASSERT_EQ_INT(OP_INPUT_MS_BTN_LEFT, pkt[6], "left button");
    ASSERT_EQ_INT(0, pkt[7], "dx=0");
    ASSERT_EQ_INT(0, pkt[8], "dy=0");
    ASSERT_EQ_INT(0, pkt[9], "wheel=0");
}
...
static void test_mouse_rel_all_buttons(void) {
    ...
    ASSERT_EQ_INT(0x07, pkt[6], "all buttons");
}

static void test_mouse_abs_coordinates(void) {
    uint8_t pkt[13];
    int len = op_input_build_mouse_abs(pkt, OP_INPUT_MS_BTN_NONE, 0x0123, 0x0456, -1);
    ASSERT_EQ_INT(13, len, "mouse abs length");
    ASSERT_EQ_INT(0x02, pkt[5], "absolute mode");
    ASSERT_EQ_INT(0x23, pkt[7], "x low");
    ASSERT_EQ_INT(0x01, pkt[8], "x high");
    ASSERT_EQ_INT(0x56, pkt[9], "y low");
    ASSERT_EQ_INT(0x04, pkt[10], "y high");
    ASSERT_EQ_INT(-1, (int8_t)pkt[11], "wheel=-1");
}

static void test_mouse_abs_header(void) {
    uint8_t pkt[13];
    op_input_build_mouse_abs(pkt, OP_INPUT_MS_BTN_NONE, 0, 0, 0);
    ASSERT_EQ_INT(0x57, pkt[0], "header 0");
    ASSERT_EQ_INT(0xAB, pkt[1], "header 1");
    ASSERT_EQ_INT(OP_INPUT_CMD_MS_ABS, pkt[3], "abs cmd");
    ASSERT_EQ_INT(0x07, pkt[4], "data len");
}
...
static void test_mouse_checksum_valid(void) {
    ...
    ASSERT_EQ_INT(op_input_checksum(rpkt, 11), rpkt[10], "rel checksum");
    ASSERT_EQ_INT(op_input_checksum(apkt, 13), apkt[12], "abs checksum");
}
...
static void test_hex_dump_empty(void) {
    ...
    ASSERT_EQ_STR("", buf, "empty hex dump");
}
...
static void test_hex_dump_single_byte(void) {
    ...
    ASSERT_EQ_STR("FF", buf, "single byte hex");
}
...
static void test_hex_dump_all_zeros(void) {
    ...
    ASSERT_EQ_STR("000000", buf, "all zeros hex");
}
...
static void test_backward_compat_constants(void) {
    ASSERT_EQ_INT(OP_INPUT_MOD_NONE,  KM_MOD_NONE,  "KM_MOD_NONE");
    ASSERT_EQ_INT(OP_INPUT_MOD_CTRL,  KM_MOD_CTRL,  "KM_MOD_CTRL");
    ASSERT_EQ_INT(OP_INPUT_MOD_SHIFT, KM_MOD_SHIFT, "KM_MOD_SHIFT");
    ASSERT_EQ_INT(OP_INPUT_MOD_ALT,   KM_MOD_ALT,   "KM_MOD_ALT");
    ASSERT_EQ_INT(OP_INPUT_MOD_GUI,   KM_MOD_GUI,   "KM_MOD_GUI");
    ASSERT_EQ_INT(OP_INPUT_CMD_KB,     KM_CMD_KB,     "KM_CMD_KB");
    ASSERT_EQ_INT(OP_INPUT_CMD_MS_REL, KM_CMD_MS_REL, "KM_CMD_MS_REL");
    ASSERT_EQ_INT(OP_INPUT_CMD_MS_ABS, KM_CMD_MS_ABS, "KM_CMD_MS_ABS");
}

static void test_backward_compat_sizes(void) {
    ASSERT_EQ_INT(OP_INPUT_PKT_KEYBOARD_SIZE,  KM_PKT_KEYBOARD_SIZE,  "KM_PKT_KEYBOARD_SIZE");
    ASSERT_EQ_INT(OP_INPUT_PKT_MOUSE_REL_SIZE, KM_PKT_MOUSE_REL_SIZE, "KM_PKT_MOUSE_REL_SIZE");
    ASSERT_EQ_INT(OP_INPUT_PKT_MOUSE_ABS_SIZE, KM_PKT_MOUSE_ABS_SIZE, "KM_PKT_MOUSE_ABS_SIZE");
}
...
static void test_checksum_overflow_boundary(void) {
    ...
    ASSERT_EQ_INT(0xFF, op_input_checksum(data1, 3), "sum=0xFF exact");
    uint8_t data2[3] = {0x80, 0x80, 0x00};
    ASSERT_EQ_INT(0x00, op_input_checksum(data2, 3), "sum=0x100 truncates");
}
...
int main(void) {
    ...
    if (test_failures != 0) {
        fprintf(stderr, "key_test_plan: %d failure(s)\n", test_failures);
        return 1;
    }

    printf("key_test_plan: all tests passed\n");
    return 0;
}
